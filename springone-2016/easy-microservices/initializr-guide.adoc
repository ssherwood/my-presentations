...
void initTravisCI(Properties props, ProjectRequest request) {
  ...
  def hooks = restTemplate.exchange("${travisUrl}/hooks", HttpMethod.PUT, new HttpEntity("{\"hook\":{\"id\":${repos.body.repo.id},\"active\":true}}", headers), Object.class)
}
```

* Select Make Project and wait for the UI to refresh.  You should now be able to
generate a project from the Initalizr that not only will create and push the
